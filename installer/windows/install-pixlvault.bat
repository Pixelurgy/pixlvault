...
set "WHEEL_FILE="
for /f "delims=" %%F in ('dir /b /o-d "%WHEEL_DIR%\pixlvault-*.whl" 2^>nul') do (
    if not defined WHEEL_FILE set "WHEEL_FILE=%%F"
)

if not defined WHEEL_FILE (
    echo Error: No pixlvault wheel found in %WHEEL_DIR%
    exit /b 1
)
...
echo.
echo PixlVault installation completed successfully.
exit /b 0
