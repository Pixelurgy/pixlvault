...
def test_smart_score_consistency():
    ...
    try:
        ...
        def setup_data(session):
            ...
            p1 = Picture(file_path="p1.jpg", image_embedding=emb_bytes, aesthetic_score=5.0, score=0)
            session.add(p1)
            
            # Picture 2: No face
            p2 = Picture(file_path="p2.jpg", image_embedding=emb_bytes, aesthetic_score=5.0, score=0)
            session.add(p2)
            ...
            # Likeness P1 -> C
            fcl = FaceCharacterLikeness(face_id=f.id, character_id=c.id, likeness=0.99, metric="cosine")
            session.add(fcl)
            session.commit()
            
            return p1.id, p2.id, c.id

        p1_id, p2_id, c_id = server.vault.db.run_task(setup_data, priority=DBPriority.IMMEDIATE)
        ...
        resp_char = client.get(f"/pictures?character_id={c_id}&sort=SMART_SCORE&descending=true")
